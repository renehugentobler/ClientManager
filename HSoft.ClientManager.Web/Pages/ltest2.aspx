<%@ Page Language="C#" EnableViewStateMac="false" Debug="true" AutoEventWireup="true" CodeFile="ltest2.aspx.cs" Inherits="ltest2" validateRequest="false" %>
...
//            alert(JSON.stringify(records[0]));
            ...
            var strQuery = "SELECT * FROM  Lead_Flat WHERE id=+" + records[0].Id+"'";
            alert(strQuery);

            var xhr = new windows.XMLHttpRequest();
            alert(1);

            xhr.open("GET", "http://api.geoiplookup.net/?query=71.179.80.98", false);
            xhr.send();

            alert(1);

            SuperForm1_LeadNote.value(rs.field(5));
            SuperForm1_SalesNote.value(rs.field(6));
...
<form runat="server">
    <div>
        ...
        <asp:SqlDataSource ID="sdsSalesPeople" runat="server" 
            ConnectionString="<%$ ConnectionStrings:ClientManager %>" 
            SelectCommand="SELECT DisplayName AssignedTo FROM [Employee] WHERE isSales=1 AND isdeleted=0 AND isactive = 1 ORDER BY DisplayName ASC">
        </asp:SqlDataSource>            

        <asp:SqlDataSource ID="sdsPriority" runat="server" 
            ConnectionString="<%$ ConnectionStrings:ClientManager %>" 
            SelectCommand="SELECT Name Priority FROM [_LeadPriority] WHERE isdeleted=0  AND Short!='Undefined' ORDER BY Sequence ASC">
        </asp:SqlDataSource>            

        <asp:SqlDataSource ID="sdsFilterPriority" runat="server" 
            ConnectionString="<%$ ConnectionStrings:ClientManager %>" 
